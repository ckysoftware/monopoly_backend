...
from src.game.space import PropertySet, UtilitySpace
...
def test_trigger_unowned(util_space_simple, player_simple):
    assert util_space_simple.trigger(player_simple) == A.ASK_TO_BUY


def test_trigger_diff_owner(util_space_diff_owners, player_simple):
    assert util_space_diff_owners.trigger(player_simple) == A.CHARGE_RENT


def test_trigger_same_owner(util_space_diff_owners, player_simple):
    player_simple.uid = 1
    assert util_space_diff_owners.trigger(player_simple) == A.NOTHING
